<template>
    <el-row class="row">
        <el-card class="register-password-user">
            <div>
                <h1 class="register-title">Tạo mới mật khẩu mật khẩu cho tài khoản</h1>
                <h1>{{username}}</h1>
            </div>
...
<script lang="ts">
    ...
    export default class ChangeForgotPassword extends Vue {
        ...
        private async onChangePassword(): Promise<void> {
            if (this.changePasswordForm.newPassword !== this.changePasswordForm.rePassword) {
                ...
                const response = await UserApi.changeNewPassword(request);
